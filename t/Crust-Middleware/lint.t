...
subtest {
    ...
    lives-ok({await $code(%env)}, 'Should work fine');
...
subtest {
    ...
    subtest {
        my $code = Crust::Middleware::Lint.new(
            sub (%env) { start { 200, 'invalid-header', ['hello'] } }
        );
        dies-ok({await $code(%env)});
    ...
    subtest {
        my $code = Crust::Middleware::Lint.new(
            sub (%env) { start { 200, ['Status' => 'Fine'], ['hello'] } }
        );
        dies-ok({await $code(%env)});
    ...
    subtest {
        {
            my $code = Crust::Middleware::Lint.new(
                sub (%env) { start { 200, ['foo:bar' => 'buz'], ['hello'] } }
            );
            dies-ok({await $code(%env)});
        }
        {
            my $code = Crust::Middleware::Lint.new(
                sub (%env) { start { 200, ['foobar-' => 'buz'], ['hello'] } }
            );
            dies-ok({await $code(%env)});
        }
        {
            my $code = Crust::Middleware::Lint.new(
                sub (%env) { start { 200, ['0foobar' => 'buz'], ['hello'] } }
            );
            dies-ok({await $code(%env)});
        }
        {
    ...
    subtest {
        my $code = Crust::Middleware::Lint.new(
            sub (%env) { start { 200, ['something' => utf8.new(0).Str], ['hello'] } }
        );
        dies-ok({await $code(%env)});
    ...
    subtest {
        my $code = Crust::Middleware::Lint.new(
            sub (%env) { start { 200, ['something' => Nil], ['hello'] } }
        );
        dies-ok({await $code(%env)});
